    ...
    <script type="text/javascript">
        ...
        function OnDsPaytoClicked(s, r, c, v) {
            if (c == "b_delpayto") {
                ...
                var assisttype_group = dsMain.GetItem(0, "assisttype_group");
                var memb_no = dsPayto.GetItem(r, "member_no");
                if (exp_code != "TRN") {
                    deptacc_no = "";
                }

                ChkRow = r;
                if (assisttype_group == "02" && exp_code == "TRN") {
                    var arg = "?memno=" + memb_no + "&exp_code=" + exp_code + "&exp_bank=" + exp_bank + "&exp_branch=" + exp_branch + "&exp_accid=" + exp_accid + "&chq_payname=" + chqpayname + "&tofrom_accid=" + tofromaccid + "&exp_clr=" + exp_clearing + "&deptaccount_no=" + deptacc_no;
                    Gcoop.OpenDlg2("500", "350", "wd_as_expense_detail_02.aspx", arg);
                }
                else {
        ...
        function OnDsPaytoItemChanged(s, r, c, v) {
            if (c == "itempay_amt") {
                ...
                var gain_name = dsPayto.GetItem(r, "gain_name");
                var count_type = 0;
...
//                    if (count_type == 0) {
                        dsPayto.SetItem(r, "description", gain_name);
                        PostGetFee();
//                    }
//                    else {
...
//                    }
        ...
        function GetValueFromDlg(bank_code, branch_code, expense_accid, pay_name, tofrom_accid, exp_clr, deptacc_no,member_no) {
            var show_text = "";
            ...
            dsPayto.SetItem(ChkRow, "member_no", member_no);
            PostPayinDesc();
        }
