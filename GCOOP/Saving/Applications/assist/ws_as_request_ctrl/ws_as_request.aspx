...
//            } 
        ...
        function OnDsMainItemChanged(s, r, c, v) {
            if (c == "member_no") {
                ...
                var ls_asstype = dsMain.GetItem(0, "assisttype_code");
                Gcoop.GetEl("HdMbChkmemdate").value = v;

                dsMain.SetItem(0, "member_no", ls_memberno);
                var pad = "00000000";
                if (v.length > 0) {
                    var mem_no = pad.substring(0, pad.length - v.length) + v;
                    Gcoop.GetEl("Hd_memno").value = mem_no;
                    Gcoop.GetEl("Hd_asstypecode").value = ls_asstype;
                }
        ...
        function OnDsAmountItemChanged(s, r, c, v) {
            ...
            } else if (c == "assistnet_amt") {
                var assistmax_amt = dsAmount.GetItem(0, "assistmax_amt");
                if (v > assistmax_amt) {
                    alert("ยอดรับเงินสุทธิมากกว่ายอดเงินสูงสุดตามรอบ กรุณาตรวจสอบ");
                    dsAmount.SetItem(0, "assistnet_amt", assistmax_amt);
                    return;
                }
                else {
                    dsAmount.SetItem(0, "assistnet_amt", v);
                }
            }
        }
...
//        }
        ...
        function Confirm() {
            debugger;
            if (confirm("สมาชิกท่านนี้มีใบคำขอสวัสดิการประเภทนี้แล้ว ต้องการแก้ไขใบคำขอใช่หรือไม่?")) {
                Gcoop.GetEl("Hd_confirmValue").value = "Yes";
                $('#ctl00_ContentPlace_btnConfirm').click();
//                PostMemberNo();

            } else {
                Gcoop.GetEl("Hd_confirmValue").value = "No";
                PostMemberNo();
            }
        }

        function open_dlg() {
            var member_no = Gcoop.GetEl("Hd_memno").value;
            var assisttype_code = Gcoop.GetEl("Hd_asstypecode").value;
            Gcoop.OpenIFrame2("600", "620", "wd_as_request_memberno.aspx", "?member_no=" + member_no + "&assisttype_code=" + assisttype_code);
        }

        function GetRefdocnoFromDlgReqOld(memberno, reqdoc_no, entry_id) {
            Gcoop.GetEl("Hd_refno").value = reqdoc_no;
            //alert(Gcoop.GetEl("Hd_refno").value);
            PostMemberNo();
        }
...
<asp:Button ID="btnConfirm" runat="server" OnClick = "OnConfirm" Text = "Raise Confirm"  Style="display: none;" />
    ...
    <asp:HiddenField ID="Hd_confirmValue" runat="server" />
    <asp:HiddenField ID="Hd_refno" runat="server"  />
    <asp:HiddenField ID="Hd_memno" runat="server"  />
    <asp:HiddenField ID="Hd_asstypecode" runat="server"  />
